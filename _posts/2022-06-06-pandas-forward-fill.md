...
데이터는 row와 column으로 구성되어 있다. row는 각각의 개별 관측치를 담고, column은 개별 관측치마다의 서로 다른(독립적인) 특징들을 담는다.

axis=1은 독립된 특징을 담는 서로 다른 column을 앞 뒤로 참조하여 채우는 방식이다. 독립된 column끼리 참조하여 값을 채운다는 것은 일반적인 사례가 아니다.
