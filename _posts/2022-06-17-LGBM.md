...
* Tree의 깊이가 깊을 수록 당연하게도 train set에 더 가까운 가중치를 학습하게 된다. 다른 모델을 사용할 때보다 현저하게 max\_depth를 줄일 필요가 있으며 max\_depth를 줄여도 test set에 대한 정확도가 올라가지 않는다면 해당 모델이 적합하지 않은 것이다.
...
lightgbm은 Classification과 Regression 모두 지원한다. 아래는 Regression 예제이다.
